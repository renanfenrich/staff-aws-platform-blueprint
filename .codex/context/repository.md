...
- Artifact contract: manual publication builds exactly once for Linux X86_64,
  ...
  provenance and SBOM attestations. ECR login and `actions/attest` use separate
  run-temporary credential paths, which are always removed without touching the
  hosted runner default. ECR Basic scanning is asynchronous advisory evidence.
